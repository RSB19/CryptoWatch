          ...
          if (typeof coin === 'string') {
            ...
            if (param === COMMAND_PRICE) {
              embed
                ...
                .addField('24h', `${change24h > 0 ? '+' : ''}${change24h.toFixed(2)}%`, true);
            } else if (param === COMMAND_DETAILS) {
              embed
                .addFields(
                  { name: 'Rank', value: `${rank}` },
                  { name: 'Market Cap', value: `$${market_cap.toLocaleString()}` },
                  { name: 'Circulating Supply', value: `${
                    circulating_supply ? circulating_supply.toLocaleString() : '--'
                  } ` },
                  { name: 'Total Supply', value: `${
                    total_supply ? total_supply.toLocaleString() : '--'
                  }` },
                  { name: 'Max Supply', value: `${max_supply ? max_supply.toLocaleString() : '--'}` },
                  { name: 'All Time High', value: `$${ath ? ath.toLocaleString() : '--'}` },
                  { name: 'Current Price', value: `$${price.toLocaleString()}` }
                )
                .addField('24h', `${change24h > 0 ? '+' : ''}${change24h.toFixed(2)}%`, true)
                .addField('30d', `${change30d > 0 ? '+' : ''}${change30d.toFixed(2)}%`, true)
                .addField('1y', `${change1y > 0 ? '+' : ''}${change1y.toFixed(2)}%`, true);
            }
